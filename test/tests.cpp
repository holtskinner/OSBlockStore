...
TEST(block_store_alloc_free_req, allocate_null) {
  ...
  ASSERT_EQ(SIZE_MAX, id)
      << "allocate should return SIZE_MAX when passed null.\n";
  score += 2;
}
...
TEST(block_store_alloc_free_req, allocate_first) {
  ...
  ASSERT_EQ(0, id) << "The id returned should be zero; this should be the "
                      "first allocation.\n";
  block_store_destroy(bs);
  score += 4;
}
...
TEST(block_store_alloc_free_req, allocate_and_free) {
  ...
  id = block_store_allocate(bs);
  ASSERT_EQ(0, id)
      << "The id returned should again be zero; did your release not work?\n";
  block_store_destroy(bs);
...
TEST(block_store, count_free_and_used) {
  ...
  // request a different arbitrary block, and used and free should update
  // accordingly.
...
TEST(block_store_serialize, valid_serialize) {
  ...
  ASSERT_EQ(bytesSerialized, BLOCK_SIZE_BYTES);
