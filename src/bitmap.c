...
struct bitmap {
  ...
  BITMAP_FLAGS flags;      // Generic place to store flags. Not enough flags to
                           // worry about width yet.
  uint8_t *data;
...
// lookup instead of always shifting bits. Should be faster? Confirmed: 10%
// faster
// Also, using native int width because it should be faster as well? -
...
static const uint8_t mask_down_inclusive[8] = {0x01, 0x03, 0x07, 0x0F,
                                               0x1F, 0x3F, 0x7F, 0xFF};

// Inverted mask
static const uint8_t invert_mask[8] = {0xFE, 0xFD, 0xFB, 0xF7,
                                       0xEF, 0xDF, 0xBF, 0x7F};

// Way more testing than I should waste my time on suggested uint8_t was faster
// but it may still be negligible/indeterminate.
// Since the data store is uint8_t, we already get punished for our bad
// alignment
...
#define B4(n) \
  B2(n)       \
  , B2(n + 1), B2(n + 1), B2(n + 2)
#define B6(n) \
  B4(n)       \
  , B4(n + 1), B4(n + 1), B4(n + 2)
static const uint8_t bit_totals[256] = {B6(0), B6(1), B6(1), B6(2)};
#undef B6
#undef B4
#undef B2
// There is an alternative for getting bit count that only loops as many times
// as there are bits set
...
size_t bitmap_ffs(const bitmap_t *const bitmap) {
  if (bitmap) {
    size_t result = 0;
    for (; result < bitmap->bit_count && !bitmap_test(bitmap, result);
         ++result) {
    }
...
size_t bitmap_total_set(const bitmap_t *const bitmap) {
  size_t total = 0;
  if (bitmap) {
    // If we have leftover, stop a byte early because we have to handle it
    // differently.
    size_t stop =
        bitmap->leftover_bits ? bitmap->byte_count - 1 : bitmap->byte_count;
    for (size_t idx = 0; idx < stop; ++idx) {
      total += bit_totals[bitmap->data[idx]];
    }
    if (bitmap->leftover_bits) {
      // haha, this is readable
      // get the byte at the end of the bitmap, mask it so we're only looking at
      // the bits in use
      // then feed that to the bit_total lookup so we don't count the bits past
      // our bit total
      // (which whould be considered undetermined)
      total += bit_totals[bitmap->data[bitmap->byte_count - 1] &
                          mask_down_inclusive[bitmap->leftover_bits - 1]];
    }
  }
  return total;
}

void bitmap_for_each(const bitmap_t *const bitmap, void (*func)(size_t, void *),
                     void *arg) {
  if (bitmap && func) {
...
bitmap_t *bitmap_initialize(size_t n_bits, BITMAP_FLAGS flags) {
  if (n_bits) {  // must be non-zero
    ...
    if (bitmap) {
      ...
      // This logic will need to be reworked when we have more than one flag,
      // haha
      // Maybe something like if (flags) and then contain a giant if/else-if for
      // each flag
